<div>
    ...
    <flux:heading size="xl" align="center">
        {{ __('auth.email-verification-needed.title') }}
    </flux:heading>

    <div class="mt-10 space-y-6 sm:mx-auto sm:w-full sm:max-w-[480px]">
        <flux:card class="space-y-6">
            <div>
                <flux:heading>{{ __('auth.email-verification-needed.heading') }}</flux:heading>
                <flux:error name="resend" />
                <flux:subheading>{{ __('auth.email-verification-needed.subheading') }}</flux:subheading>
            </div>
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="resend" variant="primary">{{ __('auth.email-verification-needed.action') }}</flux:button>
            </div>
        </flux:card>
